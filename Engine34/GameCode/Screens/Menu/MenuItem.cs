...
namespace GameCode.Screens.Menu
{
    class MenuItem
    {
     public Vector2 position;
        ...
        Vector2 screenCenter = Locator.Instance.getService<ICameraManager>().getWorldPosition(new Vector2(Game1.Instance.graphics.PreferredBackBufferWidth / 2, Game1.Instance.graphics.PreferredBackBufferHeight / 2));
        ///Unused for now
        Vector2 textCenter;
     ...
     public MenuItem(string _name, float yOffset, SpriteFont _spriteFont, int _index, Texture2D bTex)
        {
            ...
            Index = _index;
            buttonTexture = bTex;
            textureCenter = Locator.Instance.getService<ICameraManager>().getWorldPosition(new Vector2(buttonTexture.Width / 2, buttonTexture.Height / 2));
